...
export async function invitePartner(email: string) {
  ...
  const { error: inviteError } = await admin.auth.admin.inviteUserByEmail(email, {
    ...
    redirectTo: `${process.env.NEXT_PUBLIC_SITE_URL ?? process.env.VERCEL_URL ? `https://${process.env.VERCEL_URL}` : 'http://localhost:3000'}/callback`,
  });

  if (inviteError) {
    console.error('[invitePartner] invite failed:', inviteError.message);
